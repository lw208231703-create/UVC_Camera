...
bool StandardUvcProtocol::parseFrame(Frame& raw, ProcessedFrame& processed) {
...
bool StandardUvcProtocol::parseYUYV(Frame& raw, ProcessedFrame& processed) {
    ...
    cv::Mat yuv(raw.height, raw.width, CV_8UC2, raw.data.data());
    cv::Mat bgr;
...
bool StandardUvcProtocol::parseMJPEG(Frame& raw, ProcessedFrame& processed) {
    cv::Mat decoded = cv::imdecode(
        cv::Mat(1, static_cast<int>(raw.data.size()), CV_8UC1, raw.data.data()),
        cv::IMREAD_UNCHANGED);
...
bool StandardUvcProtocol::parseGray8(Frame& raw, ProcessedFrame& processed) {
    ...
    processed.data = std::move(raw.data);
    processed.timestamp_us = raw.timestamp_us;
    processed.valid = true;
    return true;
}

bool StandardUvcProtocol::parseGray16(Frame& raw, ProcessedFrame& processed) {
    size_t expected = static_cast<size_t>(raw.width) * raw.height * 2;
    if (raw.data.size() < expected) {
